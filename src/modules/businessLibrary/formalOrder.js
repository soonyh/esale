var FormalOrder = function() {
    ...
    var switchContent = function(step) {
        ...
        var tmpl = ['<button class="weui-btn weui-btn_plain-primary" onClick="FormalOrder.prev()" id="btn-prev">上一步</button>', '<button class="weui-btn weui-btn_primary" onClick="FormalOrder.next()" id="btn-next" data-loading-text="下一步...">下一步</button>', '<button class="weui-btn weui-btn_plain-primary" onClick="App.modal({url: App.modules.businessLibrary.receipt,data:\'\',reload:false})">回执</button>', '<button class="weui-btn weui-btn_primary" onClick="FormalOrder.charge()">收费</button>']
        var str = ''; //存放action-bar的内容
        switch (step) {
            case 1:
                options = {
                    url: App.modules.businessLibrary.kuanDaiXinXi,
                    container: '#tab1',
                    reload: false
                };
                str = tmpl[0] + tmpl[1];
            ...
            case 2:
                options = {
                    url: App.modules.businessLibrary.keXuanBao,
                    container: '#tab2',
                    reload: false
                };
                str = tmpl[0] + tmpl[1];
            ...
            case 3:
                options = {
                    url: App.modules.businessLibrary.dingDanXinXi,
                    container: '#tab3',
                    reload: false
                };
                str = tmpl[0] + tmpl[1];
            ...
            case 4:
                options = {
                    url: App.modules.businessLibrary.shouYinTai,
                    container: '#tab4',
                    reload: true
                };
    ...
    var handleSubmit = function(step) {
        if (+step == 1) {
            //校验1: 判断表单是否有未填项
            $('#tab1').find('.form-group input').each(function() {
                if ($(this).val() == '') {
            ...
            //拼数据
            var b = App.serialize($('#tab1').find('form').serialize());
            var d = App.serialize2('#tab1');
            if (validator) {
                $btnNext = $('#btn-next').button('loading');
                //提交
                $.ajax({
                    url: App.urls.businessLibrary.submitKuanDaiXinXi,
                    async: false,
                    type: 'post',
                    contentType:'application/json; charset=UTF-8',
                    dataType: 'json',
                    data: JSON.stringify($.extend(true, {}, {
                        token: FormalOrder.state.token
                    }, b, d)),
                }).done(function(data) {
                    if (!App.checker(data.health)) {
                        return
                    };
                    FormalOrder.state.token = data.data.token;
                    success = true;
                    validator = false;
                }).always(function() {
            ...
            if (validator) {
                ...
                $.ajax({
                    url: App.urls.businessLibrary.submitKeXuanBao,
                    async: false,
                    type: 'post',
                    contentType:'application/json; charset=UTF-8',
                    dataType: 'json',
            ...
            if (validator) {
                ...
                // })
                $.ajax({
                    url: App.urls.businessLibrary.submitDingDanXinXi,
                    async: false,
                    type: 'post',
                    contentType:'application/json; charset=UTF-8',
                    dataType: 'json',
    ...
    return {
        ...
        charge: function() {
            if (FormalOrder.state.receipt == undefined || FormalOrder.state.receipt == false) {
